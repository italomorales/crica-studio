    ...
    if (kind === 'shop') {
        ...
        if (item.status === 'published' && !a.demoListing) {
            ...
            if (!externalUrl(a.url))
                errors.push(
                    'Inclua o link do anúncio para publicar ou mantenha como demonstração.',
                );
        }
    }
    return errors;
...
export function validateType(type: CatalogType, state: CatalogState): string[] {
    ...
    if (!['shop', 'suppliers', 'both'].includes(type.scope))
        errors.push('Escolha onde usar o tipo.');
    ...
    if (type.scope === 'shop' && state.affiliates.some((p) => p.typeId === type.id))
        errors.push(
            'Este tipo é usado em Fornecedores. Mantenha a aplicação Fornecedores ou Ambos.',
        );
    return errors;
}
