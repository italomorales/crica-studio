...
export function validQuantity(value:number):boolean { return Number.isSafeInteger(value) && value>=1; }
export function buildMessage(name:string,quantity:number,idea:string):string {
 ...
 return `Olá! Vi ${name} no site da Crica Studio e gostaria de um orçamento. Quantidade: ${quantity}.`+(idea.trim()?` Minha ideia: ${idea.trim()}`:'');
}
