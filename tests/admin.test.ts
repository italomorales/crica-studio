...
test('publicação exige conteúdo e valores válidos, rascunho permite foto pendente', () => {
  ...
  assert.ok(
    validateItem({ ...product, images: [] }, 'shop', state.types).some((e) => e.includes('foto')),
  );
  assert.deepEqual(
    validateItem({ ...product, images: [], status: 'draft', description: '' }, 'shop', state.types),
    [],
  );
...
test('link obrigatório para indicação real; demonstração pode ter destino pendente', () => {
  ...
  assert.deepEqual(
    validateItem({ ...item, url: 'https://example.com/item?ref=a%2Bb' }, 'suppliers', state.types),
    [],
  );
...
test('persistência mantém alterações e links após leitura e falha sem ocultar erro de quota', () => {
  ...
  const saved = {
    ...
    products: [product],
    affiliates: [
      { id: 's', name: 'Material', description: 'Teste', url: 'https://example.com/?ref=a%2Bb' },
    ],
  };
